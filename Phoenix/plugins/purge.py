...
from Phoenix import app
from Phoenix.config import PREFIX
...
@Phoenix.on_message(filters.command("purge", PREFIX) & filters.me)
async def purge_message(client, message):
    ...
    ms_g = await client.send_message(
        message.chat.id,
        f"Phoenix Userbot purged {count_del_etion_s} messages in {time_taken_ms} seconds",
    )
    await asyncio.sleep(5)
    await ms_g.delete()


@Phoenix.on_message(filters.command("del", PREFIX) & filters.me)
