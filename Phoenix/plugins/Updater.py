...
@client.on_cmd(
    "update",
    about={
        "header": "Check Updates or Update Phoenix",
        "flags": {
            "-pull": "pull updates",
...
async def check_update(message: Message):
    ...
    if not (pull_from_repo or push_to_heroku):
        if out:
            ...
            await message.edit(f"**Phoenix is up-to-date with [{branch}]**", del_in=5)
        return
    if pull_from_repo:
        if out:
            ...
            if not push_to_heroku:
                await message.edit(
                    "**Phoenix Successfully Updated!**\n"
                    "`Now restarting... Wait for a while!`",
                    del_in=3,
                )
                asyncio.get_event_loop().create_task(Phoenix.restart(True))
        elif push_to_heroku:
            await _pull_from_repo(repo, branch)
        else:
            ...
            asyncio.get_event_loop().create_task(Phoenix.restart())
    if push_to_heroku:
...
def _heroku_helper(sent: Message, repo: Repo, branch: str) -> None:
    ...
    def progress(op_code, cur_count, max_count=None, message=""):
        ...
        if not edited or (now - start_time) > 3 or message:
            edited = True
            start_time = now
            Phoenix.loop.create_task(sent.try_to_edit(f"{cur_msg}\n\n{prog}"))
